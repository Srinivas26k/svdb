...
pub fn apply_auto_strategy(db: &mut SvDB) {
    ...
    println!("SrvDB Auto-Tuner: System RAM {} GB | Vectors {} -> Selected Mode: {:?}", ram_gb, count, best_mode);
...
pub fn check_and_migrate(db: &mut SvDB) -> anyhow::Result<()> {
    ...
    println!("SrvDB Adaptive Core: Migrating from {:?} to {:?}", current_internal_mode, new_mode);
...
fn migrate_to_sq8(db: &mut SvDB) -> anyhow::Result<()> {
    ...
    let vstorage = db.vector_storage.as_ref().unwrap();
    let count = vstorage.count();
    let dim = if count > 0 { vstorage.get(0).unwrap().len() } else { 1536 };
    ...
    let mut scalar_storage = ScalarQuantizedStorage::new_with_training(
        path_str,
        dim,
        &training_data
    )?;
